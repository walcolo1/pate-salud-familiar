      ...
      <section className="flex flex-col gap-4">
        {activeTab === 'ACTIVE' ? (
          activePrescriptions.length === 0 ? (
            <EstadoVacio
              icono={Activity}
              titulo="No hay medicamentos activos"
              descripcion="Registra tus prescripciones para calcular automáticamente tus tomas y recibir alertas de recordatorio."
              accion={{ tipo: 'boton', etiqueta: 'Registrar medicamento', onClick: () => setShowAddForm(true) }}
            />
          ) : (
          ...
          inactivePrescriptions.length === 0 ? (
            <EstadoVacio
              icono={History}
              titulo="No hay historial inactivo"
              descripcion="Aquí aparecerán los tratamientos que hayas marcado como finalizados, suspendidos o cancelados."
            />
          ) : (
