          ...
          {reminders.length === 0 ? (
            ...
            reminders.map((rem) => {
              ...
              return (
                <div 
                  key={rem.id}
                  // C1.5 · La tarjeta entera es el control: quien responde al
                  // clic es este <div>, así que es él quien tiene que responder
                  // al teclado. `role="checkbox"` porque alterna entre hecho y
                  // pendiente, y su nombre accesible sale del propio contenido
                  // de la tarjeta, sin repetir datos clínicos en un aria-label.
                  role="checkbox"
                  ...
                  onKeyDown={(e) => {
                    ...
                    // Sin esto, el espacio desplaza la página bajo el dedo.
                    e.preventDefault();
                ...
                >
                  ...
                  <span aria-hidden="true" className={`p-1.5 rounded-full shrink-0 border ${
                    isCompleted 
                      ? 'bg-teal-700 text-white border-teal-600 shadow-md shadow-teal-600/10' 
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-0.5">
                      <h4 className={`text-xs font-extrabold truncate ${isCompleted ? 'line-through text-slate-500' : 'text-slate-800'}`}>
                        {rem.title}
                      </h4>
                      <span className="text-[9px] font-extrabold bg-slate-50 text-slate-500 px-2 py-0.5 rounded-full uppercase border border-slate-200/50">
                        {patient}
                      </span>
                    </div>
                    <p className={`text-[10px] truncate mb-1.5 ${isCompleted ? 'text-slate-500' : 'text-slate-500 font-bold'}`}>{rem.description}</p>
                    
                    {/* Time limit badge */}
                    <div className="flex items-center gap-1.5">
                      <Clock className="h-3.5 w-3.5 text-teal-700 shrink-0" />
                      <span className={`text-[10px] font-extrabold ${isOverdue ? 'text-rose-700' : 'text-teal-700'}`}>
          ...
          {tasks.length === 0 ? (
            ...
            tasks.map((task) => {
              ...
              return (
                <div 
                  key={task.id}
                  // C1.5 · Igual que el recordatorio, pero aquí la acción no
                  // tiene vuelta: es un botón, no una casilla. Una tarea ya
                  // completada sale del recorrido del tabulador porque ya no
                  // hace nada al pulsarla.
                  role="button"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {task.description && (
                      <p className="text-[10px] text-slate-500 font-bold truncate mb-1.5">{task.description}</p>
                    )}
