...
export const EMISORES_VALIDOS = ['accounts.google.com', 'https://accounts.google.com'];

/** Cuánto vive una verificación en caché. */
export const CACHE_TOKEN_SEGUNDOS = 300;

/** Prefijo de la clave de caché, para no chocar con otras cosas del script. */
export const PREFIJO_CACHE_TOKEN = 'tok_';
 ...
 */
export const LONGITUD_CLAVE_CACHE = 64;

/** Dominios donde los puntos y el `+etiqueta` no cambian de buzón. */
export const DOMINIOS_CON_ALIAS = ['gmail.com', 'googlemail.com'];

/** Lo único que se devuelve de un token válido. Ni un campo más. */
export interface Identidad {
  email: string;
  sub: string;
}

/** Lo que interesa de un `id_token`, tal y como puede venir. */
 ...
 */
export const ERROR_TOKEN = 'TOKEN_INVALIDO';
 ...
 */
function base64UrlAUtf8(segmento: string): string {
 ...
 */
export function decodificarPayloadJwt(idToken: unknown): PayloadToken {
  if (typeof idToken !== 'string' || idToken.length === 0) invalido();

  const partes = (idToken as string).split('.');
  if (partes.length !== 3) invalido();
  if (partes[1].length === 0) invalido();

  try {
    const payload = JSON.parse(base64UrlAUtf8(partes[1])) as unknown;
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) invalido();
 ...
 */
function esCierto(v: unknown): boolean {
  return v === true || v === 'true';
}

/** Un `exp` utilizable, en segundos. `tokeninfo` lo manda como cadena. */
function aSegundos(v: unknown): number | null {
...
export function validarClaims(
  ...
  if (typeof audienciaEsperada !== 'string' || audienciaEsperada.length === 0) invalido();
  if (!payload || typeof payload !== 'object') invalido();
  ...
  if (!audiencias.some((a) => a === audienciaEsperada)) invalido();

  // 2 · `email_verified`. Un correo sin verificar puede ser de cualquiera.
  if (!esCierto(payload.email_verified)) invalido();
  ...
  if (exp === null || exp * 1000 <= ahoraMs) invalido();
  ...
  // hay nada que pensar.
  if (payload.iss !== undefined && !EMISORES_VALIDOS.includes(String(payload.iss))) invalido();

  const email = typeof payload.email === 'string' ? payload.email.trim() : '';
  const sub = typeof payload.sub === 'string' ? payload.sub.trim() : '';
  if (email.length === 0 || sub.length === 0) invalido();
...
export function normalizarEmail(email: unknown): string {
  ...
  if (arroba <= 0 || arroba === limpio.length - 1) return limpio;

  const dominio = limpio.slice(arroba + 1);
  if (!DOMINIOS_CON_ALIAS.includes(dominio)) return limpio;

  let local = limpio.slice(0, arroba);
  const mas = local.indexOf('+');
  if (mas !== -1) local = local.slice(0, mas);
  ...
  if (local.length === 0) return limpio;
 ...
 */
export function claveCache(idToken: unknown): string {
  if (typeof idToken !== 'string' || idToken.length === 0) invalido();

  const partes = (idToken as string).split('.');
  if (partes.length !== 3 || partes[2].length === 0) invalido();
...
/** Lo que se guarda en caché: la identidad y cuándo caduca el token. */
export interface EntradaCache {
  email: string;
  sub: string;
  /** `exp` del token, en segundos. */
  exp: number;
}

export function serializarCache(identidad: Identidad, expSegundos: number): string {
 ...
 */
export function leerCache(crudo: unknown, ahoraMs: number): Identidad | null {
  if (typeof crudo !== 'string' || crudo.length === 0) return null;

  try {
    const entrada = JSON.parse(crudo) as EntradaCache;
    const exp = aSegundos(entrada?.exp);
    if (exp === null || exp * 1000 <= ahoraMs) return null;
    if (typeof entrada.email !== 'string' || entrada.email.length === 0) return null;
    if (typeof entrada.sub !== 'string' || entrada.sub.length === 0) return null;
...
export function urlTokenInfo(idToken: string): string {
