...
import { alcanza, type Acceso, type Rol } from './acceso';
...
/**
 * Dónde se aplica un verbo.
 *
 * `FAMILIA` son acciones sobre el expediente entero: listar pacientes, mirar
 * catálogos, repartir accesos. `PACIENTE` son las que se hacen sobre alguien
 * concreto, y esas exigen identificarlo.
 */
export type AmbitoVerbo = 'FAMILIA' | 'PACIENTE';

/**
 * Para qué especie tiene sentido un verbo.
 *
 * Casi todos valen para las dos: una cita, un documento o una vacuna son lo
 * mismo en una persona y en un animal, y por eso el modelo no los duplica. Solo
 * dos no: el historial veterinario, que es de mascota, y las órdenes médicas
 * con su autorización de EPS, que son de persona.
 */
export type EspeciePaciente = 'HUMANO' | 'MASCOTA';
export type EspecieVerbo = EspeciePaciente | 'CUALQUIERA';

export interface DefinicionVerbo {
  ambito: AmbitoVerbo;
  especie: EspecieVerbo;
  /** Qué autoriza, en una línea. Va a `PERMISOS.md`. */
  descripcion: string;
}
 ...
 */
export const VERBOS: Record<string, DefinicionVerbo> = {
  // ── Familia ───────────────────────────────────────────────────────────────
  LISTAR_PACIENTES: {
    ambito: 'FAMILIA',
    especie: 'CUALQUIERA',
...
export type Verbo = keyof typeof VERBOS;

export const esVerbo = (v: unknown): v is Verbo =>
  typeof v === 'string' && Object.prototype.hasOwnProperty.call(VERBOS, v);
 ...
 */
export const MATRIZ_PERMISOS: Record<Exclude<Rol, 'TITULAR'>, readonly Verbo[]> = {
  /**
...
export const VERBOS_SOLO_TITULAR: readonly Verbo[] = [
  'ADMINISTRAR_ACCESOS',
  'VER_AUDITORIA',
  'EXPORTAR_EXPEDIENTE',
  'EDITAR_CATALOGOS',
...
const CONJUNTOS: Record<string, Set<string>> = (function construir() {
  const mapa: Record<string, Set<string>> = {};
  const roles = Object.keys(MATRIZ_PERMISOS);
  for (let i = 0; i < roles.length; i++) {
    mapa[roles[i]] = new Set(MATRIZ_PERMISOS[roles[i] as Exclude<Rol, 'TITULAR'>]);
  }
  return mapa;
})();
...
export function puede(
  ...
  if (!acceso || acceso.estado !== 'ACTIVO') return false;
  ...
  if (!esVerbo(verbo)) return false;
  const definicion = VERBOS[verbo as string];
  ...
  if (definicion.ambito === 'PACIENTE') {
    ...
    if (definicion.especie !== 'CUALQUIERA') {
      if (especiePaciente !== definicion.especie) return false;
    }

    // El cruce. Tener el verbo no basta si el paciente no está en el alcance.
    if (!alcanza(acceso, id)) return false;
  }
  ...
  if (acceso.rol === 'TITULAR') return true;

  const permitidos = CONJUNTOS[acceso.rol];
  if (!permitidos) return false;
  return permitidos.has(verbo as string);
}
 ...
 */
export function verbosDe(rol: unknown): Verbo[] {
  if (rol === 'TITULAR') return Object.keys(VERBOS);
  const permitidos = CONJUNTOS[rol as string];
  return permitidos ? Array.from(permitidos) : [];
}

/** ¿Este verbo necesita que le digan sobre qué paciente actúa? */
export function exigePaciente(verbo: unknown): boolean {
  return esVerbo(verbo) && VERBOS[verbo as string].ambito === 'PACIENTE';
}
